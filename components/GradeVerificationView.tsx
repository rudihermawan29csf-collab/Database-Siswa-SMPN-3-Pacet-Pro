...
interface GradeVerificationViewProps {
  students: Student[];
  onUpdate?: () => void;
  currentUser?: { name: string; role: string }; // To tag verifier
}
...
const GradeVerificationView: React.FC<GradeVerificationViewProps> = ({ students, onUpdate, currentUser }) => {
  ...
  const availableClasses = React.useMemo(() => {
      let level = 'VII';
      if (activeSemester >= 3 && activeSemester <= 4) level = 'VIII';
      if (activeSemester >= 5) level = 'IX';
      
      const allClasses = Array.from(new Set(students.map(s => s.className))).sort();
      // Filter classes starting with the level (e.g., "VII ")
        ...
        <div className="flex gap-3 w-full lg:w-auto items-center">
             ...
             <div className="flex items-center gap-2 bg-gray-100 px-3 py-2 rounded-lg border border-gray-200">
                ...
                <select className="bg-transparent text-sm font-bold text-gray-700 outline-none cursor-pointer" value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)}>
                    {availableClasses.length > 0 ? availableClasses.map(c => <option key={c} value={c}>{c}</option>) : <option disabled>Tidak ada kelas</option>}
                </select>
             </div>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {currentRecord.subjects.map((s, idx) => (
                                    <tr key={idx}>
                                        ...
                                        <td className="px-3 py-2 text-center font-bold">{isEditing ? (<input type="number" className="w-10 text-center border rounded bg-white focus:ring-1 focus:ring-blue-500" value={s.score} onChange={(e) => handleGradeChange(idx, e.target.value)} />) : s.score}</td>
