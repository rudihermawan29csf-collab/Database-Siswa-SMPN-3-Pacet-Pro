...
// ... (Rest of the component remains similar, updated render logic below)
...
const ReportTemplate: React.FC<ReportTemplateProps> = ({ student, semester, appSettings, userRole, onCorrectionRequest, isBatch = false }) => {
    // ... (Template logic same as existing, omitted for brevity as color logic is applied in main table)
    // Actually, report template usually is black and white for print, color is for screen dashboard.
    // The request specifies "di tab nilai siswa", which implies the dashboard view.
    // I will focus on the dashboard view table.
    
    // ... (Standard Report Template code) ...
    ...
    return (
        <div className={`bg-white ${isBatch ? '' : 'shadow-xl'} h-[296mm] w-[210mm] mx-auto p-[10mm] text-black font-sans relative print:shadow-none print:w-full print:m-0 print:p-0 box-border overflow-hidden`}>
            ...
            {/* A. INTRAKURIKULER - Scores here are usually B&W for print, but for consistency if screen view, we keep it simple or apply color if requested. The request said "tab nilai siswa", implying the dashboard view. Keeping template B&W is safer for print.*/}
            <div className="mb-4">
            ...
            {/* ... (Rest of report template same as before) ... */}
             <div className="mb-4">
...
const GradesView: React.FC<GradesViewProps> = ({ students, userRole = 'ADMIN', loggedInStudent, onUpdate }) => {
  ...
  useEffect(() => {
      if (userRole === 'STUDENT' && loggedInStudent && viewMode !== 'REPORT') {
          setViewMode('REPORT'); setSelectedStudent(loggedInStudent);
      }
  ...
  // ... (Edit & Save Handlers - same as before) ...
  ...
  // ... (Correction handlers same as before) ...
  const handleOpenClassCorrection = () => { if (selectedStudent) { setCorrectionType('CLASS'); setProposedClass(selectedStudent.className); setIsCorrectionModalOpen(true); } };
  const handleOpenGradeCorrection = (subj: string, score: number) => { setCorrectionType('GRADE'); setTargetSubject(subj); setProposedScore(String(score)); setIsCorrectionModalOpen(true); };
  
  const submitCorrection = async () => { /* ... implementation same as before ... */ setIsCorrectionModalOpen(false); };

  // ... (Excel handlers same as before) ...
             ...
             <div className="flex flex-col md:flex-row gap-3 w-full xl:w-auto">
                ...
                {userRole === 'ADMIN' && (
                    <>
                        ...
                        <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-2 px-3 py-2 rounded-lg font-bold text-sm border bg-white text-blue-600 border-blue-200 hover:bg-blue-50"><UploadCloud className="w-4 h-4" /> Import Excel</button>
                    </>
                )}
